...
+ (NSString *) desEncryptStr:(NSString *)str key:(NSString *)key
{
    ...
    NSMutableString* str1 = [NSMutableString stringWithString:[str stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
//    [XLEncrytHelper encodeSpecialUrlCharacters:str1];
    
	NSMutableString* encode = [NSMutableString stringWithString:[XLEncrytHelper doCipher:str1 key:key context:kCCEncrypt]];
    
    [XLEncrytHelper encodeSpecialUpDataCharacters:encode];
    return encode;
}
+ (NSString *) desDecryptStr:(NSString	*)str key:(NSString *)key
{
    ...
    [XLEncrytHelper encodeSpecialDownDataCharacters:str1];
    NSString *rt = [XLEncrytHelper doCipher:str1 key:key context:kCCDecrypt];
    return rt;
}
// @@—>/  $$->+
+ (NSString *)doCipher:(NSString *)sTextIn key:(NSString *)sKey
    ...
    if (encryptOrDecrypt == kCCDecrypt) {
        dTextIn = [[XLEncrytHelper decodeBase64WithString:sTextIn] mutableCopy];
    }
    else{
...
//            &movedBytes1);
    ...
    NSString * sResult;
    if (encryptOrDecrypt == kCCDecrypt){
        //sResult = [[[ NSString alloc] initWithData:[NSData dataWithBytes:bufferPtr1 length:movedBytes1] encoding:EnC] autorelease];
        ...
        free(bufferPtr1);
        sResult = [XLEncrytHelper encodeBase64WithData:dResult];
    }
    return sResult;
}

+ (NSString *)encodeBase64WithString:(NSString *)strData {
	return [XLEncrytHelper encodeBase64WithData:[strData dataUsingEncoding:NSUTF8StringEncoding]];
}
...
+ (void)encodeSpecialUpDataCharacters:(NSMutableString *)encodeStr
{
    [encodeStr replaceOccurrencesOfString:@"+" withString:@"$$" options:NSWidthInsensitiveSearch range:NSMakeRange(0, [encodeStr length])];
...
{
    return [encryptStr replaceOccurrencesOfString:@"/" withString:@"@@" options:NSWidthInsensitiveSearch range:NSMakeRange(0, [encryptStr length])];
}
...
//        [XLEncrytHelper encodeSpecialUrlCharacters:str1];
	...
	NSMutableString* encode = [NSMutableString stringWithString:[XLEncrytHelper newDoCipher:str1 key:key context:kCCEncrypt]];
...
//    NSMutableString* str1 = [NSMutableString stringWithString:[str stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    ...
    NSMutableString* encode = [NSMutableString stringWithString:[XLEncrytHelper newWithoutEncodeDoCipher:str key:key context:kCCEncrypt]];
...
+ (NSString *) newDesDecryptStr:(NSString	*)str key:(NSString *)key{
    if (str) {
        ...
        NSString *rt = [XLEncrytHelper newDoCipher:str1 key:key context:kCCDecrypt];
...
+ (NSString *)newDoCipher:(NSString *)sTextIn key:(NSString *)sKey
			   context:(CCOperation)encryptOrDecrypt {
	NSStringEncoding EnC = NSUTF8StringEncoding;
